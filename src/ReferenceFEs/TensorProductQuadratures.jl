...
struct GaussLobatto <: QuadratureName end

const tensor_product = TensorProduct()

const gauss_lobatto = GaussLobatto()
...
function Quadrature(p::Polytope,::GaussLobatto,degrees)
  @assert is_n_cube(p) """\n
  Tensor product quadrature rule only for n-cubes.
  """
  @assert length(degrees) == num_dims(p)
  _tensor_product_lobatto(degrees)
end

function Quadrature(p::Polytope,name::GaussLobatto,degree::Integer)
  degrees = ntuple(i->degree,Val(num_dims(p)))
  Quadrature(p,name,degrees)
end
...
function _tensor_product_lobatto(degrees)
    D = length(degrees)
    T = Float64
    npoints = [ ceil(Int,(degrees[i]+1.0)/2.0) for i in 1:D ]
    ...
    (coords, weights) = _tensor_product(Point{D,T},quads,npoints)
    GenericQuadrature(coords,weights,"Tensor product of 1d Gauss-Lobatto quadratures of degrees $degrees")
end
